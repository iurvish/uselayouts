...
assert.equal(maxConcurrentVideos({ hardwareConcurrency: 4, deviceMemory: 4 }), 4);
assert.equal(maxConcurrentVideos({ hardwareConcurrency: 8, deviceMemory: 8 }), 8);
assert.equal(maxConcurrentVideos({ hardwareConcurrency: 16, deviceMemory: 16 }), 10);
