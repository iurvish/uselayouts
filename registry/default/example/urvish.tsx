...
const PhotoCard = memo<PhotoCardProps>(
  ({ image, label, category, aspect, radius, widthPercent, crossfade = 0.45, panel, isDark, mode }) => {
    ...
    return (
      ...
      >
        <div
          className={`relative w-full max-h-full overflow-hidden p-1.5 transition-all duration-500 rounded-[calc(${radius}px+6px)] ${
            mode === 'dark'
          ...
          >
            ...
            {category && (
              <div className="absolute bottom-3 left-3 pointer-events-none">
                <span
                  className={`inline-flex items-center px-3 py-1 rounded-full text-[11px] font-semibold tracking-wider uppercase backdrop-blur-md shadow-sm transition-all duration-300 ${
                    isDark
                      ? 'bg-zinc-950/70 border border-zinc-800 text-zinc-200'
                      : 'bg-white/80 border border-zinc-200/80 text-zinc-900'
                  }`}
                >
                  {category}
                </span>
...
export const WheelCarousel = forwardRef<WheelCarouselRef, WheelCarouselProps>(
  (
    ...
    return (
      <motion.div
        className={`wheel-carousel-container relative w-full h-full flex items-center justify-center overflow-hidden ${className}`}
...
export const WheelCarouselDemo: React.FC = () => {
  ...
  return (
    <div
      className="relative flex h-full min-h-[560px] w-full flex-col overflow-hidden font-sans transition-colors duration-500"
    ...
    >
      ...
      <div className="flex justify-center pt-8 z-10">
        <div
          className={`inline-flex items-center gap-1 p-1 rounded-full backdrop-blur-2xl shadow-sm transition-all duration-300 ${
            isDark
              ? 'bg-zinc-900/60 border border-zinc-800/90 shadow-[0_8px_24px_rgba(0,0,0,0.5)]'
              : mode === 'custom'
              ? 'bg-[#fff6ec]/75 border border-[#e8792e]/25 shadow-[0_8px_24px_rgba(180,84,30,0.08)]'
              : 'bg-zinc-100/80 border border-zinc-200/80 shadow-[0_8px_24px_rgba(0,0,0,0.04)]'
          }`}
        >
          {modes.map(({ key, label }) => {
            ...
            return (
              <button
                key={key}
                onClick={() => setMode(key)}
                className={`relative px-5 py-2 text-sm font-medium rounded-full transition-all duration-200 outline-none ${
                  isSelected
                    ? isDark
