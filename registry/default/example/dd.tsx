...
const TESTIMONIALS: Testimonial[] = [
  {
    ...
    // Unsplash: confident latin male executive — Lucas Lenzi
    photoSrc:
  ...
  },
  {
    ...
    // Unsplash: creative south-asian woman — Christina @ wocintechchat.com
    photoSrc:
  ...
  },
  {
    ...
    // Unsplash: sharp african male professional — Joseph Gonzalez
    photoSrc:
  ...
  },
  {
    ...
    // Unsplash: approachable caucasian male in smart casual — Austin Distel
    photoSrc:
  ...
  },
  {
    ...
    // Unsplash: confident female founder — Tamara Bellis
    photoSrc:
...
// Shared spring transition (type-safe)
...
// ---------------------------------------------------------------------------
// Fade-up transition (author, stats)
// ---------------------------------------------------------------------------
...
function ProgressBar({
  ...
  const rawProgress = useMotionValue(0); // 0..1 within current segment

  // Spring-smooth the indicator's top position on slide change
  const springTop = useSpring(active * segmentH, {
    stiffness: 300,
    damping: 36,
    mass: 0.9,
  });
  ...
  useEffect(() => {
    springTop.set(active * segmentH);
  }, [active, springTop, segmentH]);

  // Height grows from 0 → segmentH driven by RAF
  ...
  useEffect(() => {
    ...
    function tick(now: number) {
      ...
      const p = Math.min(elapsed / AUTO_ROTATE_MS, 1);
      rawProgress.set(p);
  ...
  return (
    ...
    >
      {/* Filled segments for completed slides */}
      {active > 0 && (
        <div
          style={{
            position: "absolute",
            ...
            left: 0,
            width: 2,
            height: active * segmentH,
            background: "rgb(17,17,18)",
            borderRadius: 2,
          }}
        />
      )}
      {/* Active segment — grows via RAF */}
      <motion.div
        style={{
          position: "absolute",
          left: 0,
...
// ---------------------------------------------------------------------------
// Thumbnail strip — spring-scrolled
// ---------------------------------------------------------------------------
function ThumbnailStrip({
  ...
  const springY = useSpring(-(active * THUMB_STRIDE), {
    stiffness: 260,
    damping: 32,
    mass: 1,
  });
  ...
  useEffect(() => {
    springY.set(-(active * THUMB_STRIDE));
  }, [active, springY]);

  // 3 copies for infinite-loop feel
    ...
    <AnimatePresence mode="wait">
      <motion.div
        ...
        style={{
          marginTop: "auto",
          paddingTop: 22,
          borderTop: "1px solid rgb(229,228,224)",
          display: "grid",
