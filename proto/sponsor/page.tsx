...
const HOVER_VARIANTS: { name: string; mode: RibbonPatternMode }[] = [
  { name: "Cylinder", mode: "cylinder" },
  { name: "Anchor", mode: "anchor" },
  { name: "Magnetic", mode: "magnetic" },
  { name: "Ripple", mode: "ripple" },
  { name: "Shear", mode: "shear" },
];

const VARIANTS = HOVER_VARIANTS.map((v) => ({
  name: v.name,
  render: () => (
    <TiersClassic pattern={v.mode} dial dialPanel={`Hover · ${v.name}`} />
  ),
}));

export default function SponsorProtoPage() {
